...
def priceFilterPage(Request):
    if(Request.method=="POST"):
        ...
        if(mc=='All' and sc=='All' and br=='All'):
            ...
            data = Product.objects.filter(maincategory=Maincategory.objects.get(name=mc),finalprice__gte=min,finalprice__lte=max)
        elif(mc=='All' and sc!='All' and br=='All'):
            data = Product.objects.filter(subcategory=Subcategory.objects.get(name=sc),finalprice__gte=min,finalprice__lte=max)
        elif(mc=='All' and sc=='All' and br!='All'):
            data = Product.objects.filter(brand=Brand.objects.get(name=br),finalprice__gte=min,finalprice__lte=max)
        elif(mc!='All' and sc!='All' and br=='All'):
            data = Product.objects.filter(maincategory=Maincategory.objects.get(name=mc),subcategory=Subcategory.objects.get(name=sc),finalprice__gte=min,finalprice__lte=max)
        elif(mc!='All' and sc=='All' and br!='All'):
            data = Product.objects.filter(maincategory=Maincategory.objects.get(name=mc),brand=Brand.objects.get(name=br),finalprice__gte=min,finalprice__lte=max)
        elif(mc=='All' and sc!='All' and br!='All'):
            data = Product.objects.filter(subcategory=Subcategory.objects.get(name=sc),brand=Brand.objects.get(name=br),finalprice__gte=min,finalprice__lte=max)
        else:
            data = Product.objects.filter(maincategory=Maincategory.objects.get(name=mc),subcategory=Subcategory.objects.get(name=sc),brand=Brand.objects.get(name=br),finalprice__gte=min,finalprice__lte=max)
        data=data[::-1]
        return render(Request,"shop.html",{'data':data,'maincategory':maincategory,'subcategory':subcategory,'brand':brand,'mc':mc,'sc':sc,'br':br})
    else:
...
def singleProductPage(Request,num):
    ...
    relatedProducts = Product.objects.filter(brand=data.brand,maincategory=data.maincategory,subcategory=data.subcategory)
    ...
    if(Request.method=="POST"):
        ...
        if(cart):
            if(id in cart.keys()):
                ...
                cart.setdefault(id,{'id':id,'name':p.name,'color':p.color,'size':p.size,'price':p.finalprice,'qty':qty,'total':p.finalprice*qty,'pic':p.pic1.url})
        else:
            cart = {id:{'id':id,'name':p.name,'color':p.color,'size':p.size,'price':p.finalprice,'qty':qty,'total':p.finalprice*qty,'pic':p.pic1.url}}
...
def forgetPasswordPage2(Request):
    if(Request.method=="POST"):
        username = Request.session.get("reset-password-username",None)
        if(username):
            try:
